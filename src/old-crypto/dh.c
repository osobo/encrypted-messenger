...
{
  // TODO: The exponent (priv) shouldn't have to be smaller than n, right?
  u32 i;
  for (i = 0; i < R_LEN; ++i) {
...
{
  // pub = g ^ priv (mod n)
  bigu_mont_pow(gm, priv->limbs, R_LEN, ctx, pub->limbs);
}

// Given a priv key and another party's pub key: calc shared secret.
// len of ss should obviously be R_LEN * LIMB_BYTE_COUNT.
int dh_calc_shared_secret(const DhPrivKey* priv, const DhPubKey* pub, u8* ss)
{
  Limb x[R_LEN];
  Limb y[R_LEN];

  // Perform actual calc. Store res in x.
  bigu_mont_pow(pub->limbs, priv->limbs, R_LEN, ctx, x);

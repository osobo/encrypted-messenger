...
void chacha20(const ChaChaCtx* ctx, u8 out[])
{
  ...
  for (k = 10; k-- > 0;) {
    DOUBLE_ROUND(a, b, c, d, tmp);
  }

  // TODO: SUPER IMPORTANT!!!
  // TODO: Deal with endianness. See salsa paper or openssl impl for example.
#if defined(LITTLE_ENDIAN)
  u32* out_words = (u32*)out;
  for (k = 16; k-- > 0;) {
    out_words[k] = ctx->words[k] + tmp[k];
  }
#elif defined(BIG_ENDIAN)
#error "Big endian not yet implementet. Big TODO!!"
#else
#error "No endianness deifned, BAD! Should be done in machine_defs.h"
#endif // defined LITTLE/BIG_ENDIAN
}

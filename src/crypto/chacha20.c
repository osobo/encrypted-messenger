...
{
  static const char const_str[] = "expand 32-byte k";
  u8 i;
  // Init the 4 first words in the matrix
  for (i = 0; i < 4; ++i) {
    ctx->words[i] = little_endian_to_word((u8*)(const_str + 4*i));
  }
}
...
//#define COMPILE_CHACHA_TESTS

#ifdef COMPILE_CHACHA_TESTS
// Tests from
...
{
  /*
  print_buffer_start(0, ((u8*)(ctx->words)) + 0, 0);
  print_buffer_start(1, ((u8*)(ctx->words)) + 8, 0);
  print_buffer_start(2, ((u8*)(ctx->words)) + 16, 0);
  print_buffer_start(3, ((u8*)(ctx->words)) + 24, 0);
  display_update();
  */
